...
export default function EditInvoicePage() {
  ...
  // Load Settings & Existing Invoice Data
  useEffect(() => {
    async function loadData() {
      try {
        ...
        if (Array.isArray(settingsData)) {
          ...
          settingsData.forEach((s: RouteSetting) => {
            map[s.route] = s;
          });
          setSettingsMap(map);
        }
        ...
        if (!invRes.ok) throw new Error('Invoice tidak ditemukan');
        const inv = await invRes.json();

        setInvoiceNumber(inv.invoice_number);
        setCustomerName(inv.customer_name);
        setRoute(inv.route);
        setWeightKg(inv.weight_kg.toString());
        setItemCount(inv.item_count.toString());
        setPickupOrDelivery(inv.pickup_or_delivery);
        setApplyPickupDiscount(inv.pickup_discount_per_kg > 0 || inv.pickup_or_delivery === 'pickup');
        setPaymentCurrencyPreference(inv.payment_currency_preference || 'ORIGINAL');
        setCustomerNote(inv.customer_note || '');
        setInternalLabelColor(inv.internal_label_color || '');
        setShowLabelToCustomer(inv.show_label_to_customer);
        setPaymentStatus(inv.payment_status);
        setDeliveryStatus(inv.delivery_status);

        if (Array.isArray(inv.extra_charges)) {
          setExtraCharges(
            inv.extra_charges.map((ec: any) => ({
              id: ec.id,
              name: ec.name,
        ...
        setErrorMsg(err.message || 'Gagal memuat data invoice');
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleDownloadPDF = () => {
    const doc = generateInvoicePDF({
      invoiceNumber: invoiceNumber || 'INV-EDIT',
      createdAt: new Date(),
      customerName: customerName || 'Nama Customer',
      route,
      weightKg: parsedWeight,
    ...
    doc.save(`Invoice_${invoiceNumber}_${customerName || 'Customer'}.pdf`);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(data.error || 'Gagal meng-update invoice');
      }

      router.push('/');
    } catch (err: any) {
      setErrorMsg(err.message || 'Terjadi kesalahan saat meng-update invoice.');
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-20">
        ...
        <span className="ml-3 text-slate-400 font-medium">Memuat invoice #{invoiceId}...</span>
      </div>
    );
  }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center space-x-3">
            <h1 className="text-2xl font-bold text-white tracking-tight">Edit Invoice</h1>
            <span className="px-3 py-1 bg-sky-950 text-sky-400 border border-sky-800 rounded-lg text-xs font-mono font-bold">
              {invoiceNumber}
            </span>
          </div>
          <p className="text-slate-400 text-sm mt-1">
            Ubah rincian pengiriman, status pembayaran, atau tambahan titip barang.
          </p>
        </div>
        <div className="flex items-center space-x-3">
          <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="misal: Merah / Dokument"
                  className="w-full bg-slate-950 border border-slate-700 rounded-xl px-4 py-2.5 text-white font-medium focus:ring-2 focus:ring-sky-500 outline-none"
                />
              </div>
          ...
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-4">
            ...
            {extraCharges.length === 0 ? (
              <div className="text-center py-6 border-2 border-dashed border-slate-800 rounded-xl text-slate-500 text-sm">
                Belum ada tambahan titip barang.
              </div>
            ) : (
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-4 sticky top-20">
            ...
            >
              ...
              <span>{saving ? 'Menyimpan Perubahan...' : 'Update Invoice'}</span>
            </button>
          </div>
        </div>
